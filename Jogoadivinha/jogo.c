...
void jogar() {
    ...
    printf("\nOla, %s! Vamos comecar o jogo? vamos!\n", jogador.nome);
    ...
    for (int rodada = 1; rodada <= 5; rodada++) {
        ...
        if (palpite == sorteada.valor) {
            printf("Parabens! Voce acertou! :D, continue assim!\n");
            jogador.pontos++;
        } else {
            printf("Eita, voce errou. :( mas nao desista!\n");
        }
    }
...
int main() {
    int opcao;
    do {
        printf("\n=== Ola e Seja Bem-vindo ao Jogo de Adivinhacao de Cartas! ===\n");
        printf("O que voce deseja fazer?\n");
        printf("1. Jogar\n2. Exibir placar\n3. Sair\n");
        printf("Digite o numero da opcao desejada de 1 a 3por favor: ");
        scanf("%d", &opcao);
        switch (opcao) {
            case 1: 
